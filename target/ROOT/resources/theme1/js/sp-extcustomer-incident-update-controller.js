...
chrisApp.controller('spExtIncidentUpdateController',  ['$rootScope', '$scope', '$filter','siteService','authService',
		...
		$scope.getIncidentSelected=function(){
			...
			ticketService.getSelectedTicketFromSession("EXTCUST")
			.then(function(data){
				console.log(data)
				if(data.statusCode == 200){
					...
					if(viewMode.toUpperCase()=="EDIT"){
						...
						$scope.getStatus();
						 $scope.ticketData.selectedExtCustomer = externalCust;
		...
		$scope.getAsset=function(selectedSite){
		 ...
		 $scope.setTicketPriorityAndSLA=function(ticketCategory){
			 ...
			 if(viewMode.toUpperCase()=='EDIT'){
				 var externalCust = $.jStorage.get('selectedExtCustomer');
				 $scope.ticketData.selectedExtCustomer = externalCust;
				 spId=parseInt(externalCust.companyId);
		      }
			 if(spId==undefined){
				 //alert("No Site Selected");
				 return false;
				 ...
				 $('#loadingDiv').show();
			 ticketService.getTicketPriorityAndSLA(spId,ticketCategory.categoryId,'EXTCUST')
			 .then(function(data){
				 //console.log(data);
				 if(data.statusCode == 200){
		 ...
		 $scope.persistTicket=function(ticketData, type){
			 $('#loadingDiv').show();
			 ticketService.saveTicket(ticketData,"EXTCUST")
			 .then(function(data){
				 //console.log(data);
				 if(data.statusCode == 200){
					 	$scope.successMessage = data.message;
