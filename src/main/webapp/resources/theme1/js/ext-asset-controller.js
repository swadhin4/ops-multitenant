		  ...
		  function  ($rootScope, $scope , $filter,authService,userService,
			...
			/* $scope.send = function(assetId){
			 ...
			 $scope.displayExternalCustomerView=function(viewType){
				...
				$scope.getCustomerSelected=function(t, e, selectBoxId ){
					...
					if(custId=="0"){
						$scope.getErrorMessage("Please select a customer from the list");
					}else{
			...
			/* $scope.addService=function(){
			 ...
			 $scope.openAssetPage=function(pageType){
				 if(pageType == 'E'){
					 window.location.href=hostLocation+"/asset/equipment/externalcustomer/add"
				 }
				 else if(pageType == 'S'){
					 window.location.href=hostLocation+"/asset/service/externalcustomer/add"
				 }				 
				 else if($scope.selectedAsset.assetType == 'E'){
					 window.location.href=hostLocation+"/asset/equipment/externalcustomer/edit"
				 }
				 else if($scope.selectedAsset.assetType == 'S'){
					 window.location.href=hostLocation+"/asset/service/externalcustomer/edit"
				 }
