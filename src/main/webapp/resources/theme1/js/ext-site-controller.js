...
chrisApp.controller('extSiteController',  ['$rootScope', '$scope', '$filter','siteService','authService',
			...
			.then(function(data) {
    			//console.log(data)
    				...
    				if(data.length>0){
    					...
    					$scope.siteData.customer=selectedCustomer;
	     ...
	     $scope.createSiteVOObject=function(finalSiteObj){
	    	...
	    	if(validateSiteAndLicense){
	            ...
	            function(data) {
	    		 ...
	    		 $scope.modalErrorMessage = "Please verify site attachments.";
 				 $scope.getErrorMessage($scope.modalErrorMessage);
 				$('#loadingDiv').hide();
	    	}
		 ...
		 $scope.getSelectedSiteData=function(siteId){
			 siteService.retrieveSiteDetails(siteId)
	    		.then(function(data) {
					...
					$scope.selectedSite.district = district;
					$scope.selectedSite.area=area;
					$scope.selectedSite.cluster=cluster;
