		 ...
		 ['$rootScope', '$scope', '$filter','authService','userService',
		  ...
		  function  ($rootScope, $scope , $filter,authService,userService,
			 ...
			 angular.element(document).ready(function () {
				 $scope.operation = $('#mode').val().toUpperCase();
				 $scope.originateFrom = $.jStorage.get('originateFrom');
				 $('#equipmentcreateWindow').hide();
				 $('#equipmentcreateWindow').show( "slide", { direction: "right"  }, 500 );
				 $scope.getLoggedInUserAccess();
				 $scope.selectedSiteRows = [];
					$("#drpIsAsset").change(
							function() {
					...
					$("#drpIsPowersensor").change(
					 ...
					 if ($scope.operation == 'NEW'){
						 ...
						 $scope.selectedAsset.customer=externalCust;
			 ...
			 function editAsset(){
				 ...
				 console.log($scope.selectedAsset);
				 var selectedSite = {
						 siteId:$scope.selectedAsset.siteId,
						 siteName:$scope.selectedAsset.siteName,
						 contactName:$scope.selectedAsset.contactName,
						 email:$scope.selectedAsset.email,
						 siteOwner:$scope.selectedAsset.siteOwner
				 }
			 ...
			 $scope.getUserSiteAccess=function(){
				 $('#loadingDiv').show();
				 var externalCust = $.jStorage.get('selectedExtCustomer');
				 $scope.accessSite.selected.customer=externalCust;
				 userService.getExternalCustSites(externalCust)
					.then(function(data) {
		    			console.log(data);
			  ...
			  $scope.selectSite = function(item) {
			    ...
			    $scope.getAllSelectedSiteRows();
			  }

			  $scope.getAllSelectedSiteRows = function() {
			    var selectedRows = $filter("filter")($scope.accessSite.list, {
			      selected: true
			    }, true);
			    
			    $scope.selectedSiteRows = selectedRows;
			  }
		        	  ...
		        	  $scope.equipmentData.categoryId=$scope.assetCategory.selected.assetCategoryId;
						...
						// $scope.equipmentData.subCategoryId2 = $scope.subRepairType.selected.subCategoryId2;
						 ...
						 if(externalCust!=null){
							 $scope.equipmentData.extCustId=parseInt(externalCust.customerId);
							 $scope.equipmentData.spType="EXTCUST";
						 }
