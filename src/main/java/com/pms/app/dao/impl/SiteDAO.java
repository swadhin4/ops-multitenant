...
public class SiteDAO {
	...
	public List<CreateSiteVO> getSiteList(String username) {
		...
		List<CreateSiteVO> siteList = jdbcTemplate.query(AppConstants.USER_SITE_ACCESS_QUERY, new Object[] { username },
				new ResultSetExtractor<List<CreateSiteVO>>() {
					@Override
	...
	public List<CreateSiteVO> getSPSiteList(String rspCode, LoginUser user) {
		...
		if (rspCode.equalsIgnoreCase(user.getCompany().getCompanyCode())) {
			JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
			siteList = jdbcTemplate.query(AppConstants.RSP_SITE_DETAILS_BY_CODE_QUERY, new Object[] { rspCode },
					new ResultSetExtractor<List<CreateSiteVO>>() {
						@Override
						public List<CreateSiteVO> extractData(ResultSet rs) throws SQLException, DataAccessException {
							List<CreateSiteVO> siteList = new ArrayList<CreateSiteVO>();
	...
	public CreateSiteVO getSiteDetails(Long siteId) {
		...
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		CreateSiteVO siteVO = jdbcTemplate.query(AppConstants.SITE_DETAILS_QUERY, new Object[] { siteId },
				new ResultSetExtractor<CreateSiteVO>() {
					@Override
					public CreateSiteVO extractData(ResultSet rs) throws SQLException, DataAccessException {
						...
						if (rs.next()) {
							siteVO.setSiteId(rs.getLong("site_id"));
							siteVO.setSiteName(rs.getString("site_name"));
							siteVO.setDistrictId(StringUtils.isEmpty(rs.getString("district_id")) == true ? null
									: Long.parseLong(rs.getString("district_id")));
							siteVO.setDistrictName(rs.getString("district_name"));
							siteVO.setAreaId(StringUtils.isEmpty(rs.getString("area_id")) == true ? null
									: Long.parseLong(rs.getString("area_id")));
							siteVO.setAreaName(rs.getString("area_name"));
							siteVO.setClusterId(StringUtils.isEmpty(rs.getString("cluster_id")) == true ? null
									: Long.parseLong(rs.getString("cluster_id")));
							siteVO.setClusterName(rs.getString("cluster_name"));
							siteVO.setSiteOwner(rs.getString("site_owner"));
							siteVO.setOwner(rs.getString("site_owner"));
							siteVO.setBrandId(rs.getLong("brand_id"));
	...
	public CreateSiteVO saveSite(CreateSiteVO siteVO, LoginUser user) {
		...
		if (siteVO.getSiteId() == null) {
			...
			LOGGER.info("Site created for " + user.getUsername() + "/ Site : " + siteVO.getSiteName() + " with ID : "
					+ siteVO.getSiteId());
			if (siteVO.getSiteId() != null) {
				insertJdbcCall = new SimpleJdbcCall(jdbcTemplate).withProcedureName("uspUserSiteAccess");
				SqlParameterSource map2 = new MapSqlParameterSource().addValue("userId", user.getUserId())
						.addValue("siteId", siteVO.getSiteId()).addValue("createdBy", user.getUsername())
						.addValue("dbName", user.getDbName()).addValue("isError", "@isError");
				Map<String, Object> out2 = insertJdbcCall.execute(map2);
				LOGGER.info("Updating SiteUserAccess table for new site created by : " + user.getUsername()
						+ "/ Site : " + siteVO.getSiteName());
			...
			LOGGER.info("Site updated for " + user.getUsername() + "/ Site : " + siteVO.getSiteName() + " with ID : "
					+ siteVO.getSiteId());
		}
	...
	private SqlParameterSource updateExistingSiteParam(CreateSiteVO siteVO, LoginUser user) {
		SqlParameterSource map = new MapSqlParameterSource().addValue("siteId", siteVO.getSiteId())
				.addValue("siteName", siteVO.getSiteName())
				.addValue("siteOwner", StringUtils.isEmpty(siteVO.getOwner()) == true ? null : siteVO.getOwner())
				.addValue("operatorId", user.getCompany().getCompanyId()).addValue("districtId", siteVO.getDistrictId())
				.addValue("areaId", siteVO.getAreaId()).addValue("clusterId", siteVO.getClusterId())
				.addValue("elecId", siteVO.getElectricityId())
				.addValue("siteNumber1", Long.parseLong(siteVO.getSiteNumber1()))
				.addValue("siteNumber2",
						StringUtils.isEmpty(siteVO.getSiteNumber2()) == true ? null
								: Long.parseLong(siteVO.getSiteNumber2()))
				.addValue("attachmentPath",
						siteVO.getSiteAttachments().isEmpty() == true ? null : siteVO.getSiteAttachments().get(0))
				.addValue("salesArea",
						StringUtils.isEmpty(siteVO.getSalesAreaSize()) == true ? null : siteVO.getSalesAreaSize())
				.addValue("brandId", siteVO.getBrandId()).addValue("brandName", siteVO.getBrandName())
				.addValue("siteContactName", siteVO.getContactName()).addValue("siteEmail", siteVO.getEmail())
				...
				.addValue("pContactNumber",
						StringUtils.isEmpty(siteVO.getPrimaryContact()) == true ? null
								: Long.parseLong(siteVO.getPrimaryContact()))
				.addValue("sContactNumber",
						StringUtils.isEmpty(siteVO.getSecondaryContact()) == true ? null
								: Long.parseLong(siteVO.getSecondaryContact()))
				.addValue("addressLine1", siteVO.getSiteAddress1()).addValue("addressLine2", siteVO.getSiteAddress2())
				.addValue("addressLine3", siteVO.getSiteAddress3()).addValue("addressLine4", siteVO.getSiteAddress4())
				.addValue("zipCode", siteVO.getZipCode()).addValue("dbName", user.getDbName())
				.addValue("modifiedBy", user.getUsername()).addValue("isError", "@isError");
		return map;
	}
	...
	private SqlParameterSource createNewSiteParam(CreateSiteVO siteVO, LoginUser user) {
		SqlParameterSource map = new MapSqlParameterSource().addValue("siteName", siteVO.getSiteName())
				...
				.addValue("addressLine3", siteVO.getSiteAddress3()).addValue("addressLine4", siteVO.getSiteAddress4())
				.addValue("zipCode", siteVO.getZipCode()).addValue("dbName", user.getDbName())
				.addValue("createdBy", user.getUsername()).addValue("siteId", "@siteId")
	...
	public List<SiteLicenceVO> getSiteLicense(Long siteId) {
		...
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		List<SiteLicenceVO> siteLicenseVOList = jdbcTemplate.query(AppConstants.SITE_LICENSE_QUERY,
				new Object[] { siteId }, new ResultSetExtractor<List<SiteLicenceVO>>() {
					@Override
	...
	public List<SiteOperationVO> getSiteSalesTimings(Long siteId) {
		...
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		List<SiteOperationVO> siteSalesTimeList = jdbcTemplate.query(AppConstants.SITE_OPERATING_SALES_TIMINGS_QUERY,
				new Object[] { siteId }, new ResultSetExtractor<List<SiteOperationVO>>() {
					@Override
	...
	public List<SiteDeliveryVO> getSiteDeliveryTimings(Long siteId) {
		...
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		List<SiteDeliveryVO> siteDeliveryTimeList = jdbcTemplate.query(
				AppConstants.SITE_OPERATING_DELIVERY_TIMINGS_QUERY, new Object[] { siteId },
				new ResultSetExtractor<List<SiteDeliveryVO>>() {
					@Override
	...
	public List<SiteSubmeterVO> getSiteSubmeterList(Long siteId) {
		...
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		List<SiteSubmeterVO> siteSubmeterList = jdbcTemplate.query(AppConstants.SITE_SUBMETER_QUERY,
				new Object[] { siteId }, new ResultSetExtractor<List<SiteSubmeterVO>>() {
					@Override
	...
	public SiteLicenceVO saveLicenseDetails(Long siteId, SiteLicenceVO siteLicence, LoginUser user) {
		...
		SqlParameterSource map = new MapSqlParameterSource().addValue("siteId", siteId)
				.addValue("licenseName",
						StringUtils.isEmpty(siteLicence.getLicenseName()) == true ? null : siteLicence.getLicenseName())
				.addValue("startDate", siteLicence.getValidfrom()).addValue("endDate", siteLicence.getValidto())
				.addValue("attachmentPath",
						StringUtils.isEmpty(siteLicence.getAttachment()) == true ? null : siteLicence.getAttachment())
				.addValue("createdBy", user.getUsername()).addValue("dbName", user.getDbName())
				.addValue("licenseId", "@licenseId").addValue("isError", "@isError");
		jdbcCall.compile();
		...
		int[] insertedRows = jdbcTemplate.batchUpdate("call tenants.uspInsertSiteLicense(?,?,?,?,?,?,?)",
				new BatchPreparedStatementSetter() {
					...
					public void setValues(PreparedStatement ps, int i) throws SQLException {
						...
						try {
							ps.setLong(1, siteId);
							ps.setString(2, StringUtils.isEmpty(siteLicence.getLicenseName()) == true ? null
									: siteLicence.getLicenseName());
							...
							ps.setString(5, StringUtils.isEmpty(siteLicence.getAttachment()) == true ? null
									: siteLicence.getAttachment());
				...
				new BatchPreparedStatementSetter() {
					...
					public void setValues(PreparedStatement ps, int i) throws SQLException {
						...
						try {
							...
							ps.setLong(2, siteId);
							ps.setString(3, StringUtils.isEmpty(siteLicence.getLicenseName()) == true ? null
									: siteLicence.getLicenseName());
							Date startDate = simpleDateFormat.parse(siteLicence.getValidfrom());
							Date endDate = simpleDateFormat.parse(siteLicence.getValidto());
							ps.setDate(4, ApplicationUtil.getSqlDate(startDate));
							ps.setDate(5, ApplicationUtil.getSqlDate(endDate));
							ps.setString(6, StringUtils.isEmpty(siteLicence.getAttachment()) == true ? null
									: siteLicence.getAttachment());
	...
	public int saveOrUpdateOeratingTimings(Long siteId, List<SiteOperationVO> siteOperation,
			List<SiteDeliveryVO> siteDeliveryList, LoginUser user, String mode) {
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		String query1 = "";
		String query2 = "";
		...
		int[] insertedRows = jdbcTemplate.batchUpdate(query1, new BatchPreparedStatementSetter() {
		...
		int[] deliveryRows = jdbcTemplate.batchUpdate(query2, new BatchPreparedStatementSetter() {
	...
	public int saveOrUpdateSubmeterBatch(Long siteId, List<SiteSubmeterVO> siteSubmterVOList, LoginUser user,
		...
		int[] insertedRows = jdbcTemplate.batchUpdate(query, new BatchPreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps, int i) throws SQLException {
				...
				ps.setLong(1, siteId);
				ps.setString(2, StringUtils.isEmpty(siteSubmeter.getSubMeterNumber()) == true ? null
						: siteSubmeter.getSubMeterNumber());
				ps.setString(3, StringUtils.isEmpty(siteSubmeter.getSubMeterUser()) == true ? null
						: siteSubmeter.getSubMeterUser());
				if (mode.equalsIgnoreCase("ADD")) {
		...
		int[] insertedRows = jdbcTemplate.batchUpdate("call tenants.uspInsertSiteSubmeter(?,?,?,?,?,?)",
				new BatchPreparedStatementSetter() {
					...
					public void setValues(PreparedStatement ps, int i) throws SQLException {
						...
						ps.setLong(2, siteId);
						ps.setString(3, StringUtils.isEmpty(siteSubmeter.getSubMeterNumber()) == true ? null
								: siteSubmeter.getSubMeterNumber());
						ps.setString(4, StringUtils.isEmpty(siteSubmeter.getSubMeterUser()) == true ? null
								: siteSubmeter.getSubMeterUser());
	...
	private Map<String, String> extractResult(Map<String, Object> out) {
		...
		String[] keyValuePairs = result1.split(","); // split the string to
		...
		for (String pair : keyValuePairs) // iterate over the pairs
		{
			String[] entry = pair.split("="); // split the pairs to get key and
												// value
			resultMap.put(entry[0].trim(), entry[1].trim()); // add them to the
																// hashmap and
																// trim
																// whitespaces
		}
	...
	public List<CreateSiteVO> getSiteListByCompany(String custCode) {
		...
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		List<CreateSiteVO> siteList = jdbcTemplate.query(AppConstants.SITE_LIST_BY_COMPANY_QUERY,
				new Object[] { custCode }, new ResultSetExtractor<List<CreateSiteVO>>() {
					@Override
					public List<CreateSiteVO> extractData(ResultSet rs) throws SQLException, DataAccessException {
						List<CreateSiteVO> siteList = new ArrayList<CreateSiteVO>();
						while (rs.next()) {
	...
	public int deleteFromDB(Long siteId) {
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		int updatedRows = jdbcTemplate.update(AppConstants.SITE_ATTACHMENT_DELETE_QUERY, new PreparedStatementSetter() {
			@Override
	...
	public int updateAttachmentSite(CreateSiteVO siteVO, LoginUser user) {
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		int updatedRows = jdbcTemplate.update(AppConstants.SITE_ATTACHMENT_UPDATE_QUERY, new PreparedStatementSetter() {
			@Override
			public void setValues(PreparedStatement ps) throws SQLException {
	...
	public CreateSiteVO saveRSPExternalCustomerSite(CreateSiteVO siteVO, LoginUser user) {
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
				final PreparedStatement ps = connection.prepareStatement(
						RSPCustomerConstants.INSERT_EXT_CUSTOMER_SITE_CREATE, Statement.RETURN_GENERATED_KEYS);
				return populateSiteParams(siteVO, user, ps);
			}

		}, key);
		LOGGER.info("Saved  External Customer Site {} with Site ID {}.", siteVO.getSiteName(), key.getKey());
		siteVO.setSiteId(key.getKey().longValue());
		siteVO.setStatus(201);
		return siteVO;
	}

	private PreparedStatement populateSiteParams(CreateSiteVO siteVO, LoginUser user, final PreparedStatement ps)
			throws SQLException {
		ps.setString(1, siteVO.getSiteName());
		ps.setString(2, StringUtils.isEmpty(siteVO.getOwner()) == true ? null : siteVO.getOwner());
		ps.setLong(3, siteVO.getCompanyId());
		if (siteVO.getDistrictId() == null) {
			ps.setNull(4, Types.NULL);
		} else {
			ps.setLong(4, siteVO.getDistrictId());
		}
		if (siteVO.getAreaId() == null) {
			ps.setNull(5, Types.NULL);
		} else {
			ps.setLong(5, siteVO.getAreaId());
		}
		if (siteVO.getClusterId() == null) {
			ps.setNull(6, Types.NULL);
		} else {
			ps.setLong(6, siteVO.getClusterId());
		}
		ps.setString(7, siteVO.getElectricityId());
		ps.setLong(8, Long.parseLong(siteVO.getSiteNumber1()));
		if (StringUtils.isEmpty(siteVO.getSiteNumber2())) {
			ps.setLong(9, Types.NULL);
		} else {
			ps.setLong(9, Long.parseLong(siteVO.getSiteNumber2()));
		}
		ps.setString(10, siteVO.getSiteAttachments().isEmpty() == true ? null : siteVO.getSiteAttachments().get(0));
		ps.setString(11, StringUtils.isEmpty(siteVO.getSalesAreaSize()) == true ? null : siteVO.getSalesAreaSize());
		ps.setLong(12, siteVO.getBrandId());
		ps.setString(13, siteVO.getBrandName());
		ps.setString(14, siteVO.getContactName());
		ps.setString(15, siteVO.getEmail());
		ps.setString(16, StringUtils.isEmpty(siteVO.getLatitude()) == true ? null : siteVO.getLatitude());
		ps.setString(17, StringUtils.isEmpty(siteVO.getLongitude()) == true ? null : siteVO.getLongitude());
		ps.setLong(18, StringUtils.isEmpty(siteVO.getPrimaryContact()) == true ? null
				: Long.parseLong(siteVO.getPrimaryContact()));
		if (StringUtils.isEmpty(siteVO.getSecondaryContact())) {
			ps.setLong(19, Types.NULL);
		} else {
			ps.setLong(19, Long.parseLong(siteVO.getSecondaryContact()));
		}
		ps.setString(20, siteVO.getSiteAddress1());
		ps.setString(21, siteVO.getSiteAddress2());
		ps.setString(22, siteVO.getSiteAddress3());
		ps.setString(23, siteVO.getSiteAddress4());
		ps.setString(24, siteVO.getZipCode());
		ps.setString(25, user.getUsername());
		if(siteVO.getSiteId()!=null){
			ps.setLong(26, siteVO.getSiteId());
		}
		return ps;
	}

	public int updateRSPExternalCustomerSite(CreateSiteVO siteVO, LoginUser user) throws Exception {
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		int updatedRow = jdbcTemplate.update(RSPCustomerConstants.UPDATE_EXT_CUSTOMER_SITE_QUERY,
				new PreparedStatementSetter() {
					@Override
					public void setValues(PreparedStatement ps) throws SQLException {
						ps=populateSiteParams(siteVO, user,  ps);
					}
				});
		return updatedRow;
	...
	public int updateRSPUserExtCustSiteAccess(LoginUser user, Long siteId) {
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		KeyHolder key = new GeneratedKeyHolder();
		jdbcTemplate.update(new PreparedStatementCreator() {
			@Override
			public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
				final PreparedStatement ps = connection.prepareStatement(RSPCustomerConstants.EXT_CUST_SP_USER_ACCESS,
		...
		LOGGER.info("Saved  User external customer Site Access for {} with Access ID {}.", user.getUsername(),
				key.getKey());
	...
	public List<CreateSiteVO> getExtCustUserSiteList(String username, Long extCustId) {
		LOGGER.info("SiteDAO -- getSiteList -- Getting Site List for User : " + username);
		JdbcTemplate jdbcTemplate = new JdbcTemplate(ConnectionManager.getDataSource());
		List<CreateSiteVO> siteList = jdbcTemplate.query(RSPCustomerConstants.EXT_CUST_USER_SITE_ACCESS_QUERY,
				new Object[] { username, extCustId }, new ResultSetExtractor<List<CreateSiteVO>>() {
					@Override
					public List<CreateSiteVO> extractData(ResultSet rs) throws SQLException, DataAccessException {
						List<CreateSiteVO> siteList = new ArrayList<CreateSiteVO>();
						while (rs.next()) {
